...
public class AutoSuggest extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (versions == null) {
            // default is v103
            request.getRequestDispatcher("/v103AS").forward(request, response);
        
        } else if (versions[0].equals("101")) {
            request.getRequestDispatcher("/v101AS").forward(request, response);                        
        } else if (versions[0].equals("102")) {
            request.getRequestDispatcher("/v102AS").forward(request, response);                                    
